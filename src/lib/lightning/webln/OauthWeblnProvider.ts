...
import type {KeysendRequestParams, OAuthClient} from "../oauth/types"
  ...
  async makeInvoice(params: RequestInvoiceArgs) {
    ...
    try {
      ...
      const result = await this.client.createInvoice({
        amount: parseInt(params.amount.toString(), 10),
    ...
    return new Promise((resolve, reject) => {
      ...
      window.addEventListener("message", async message => {
        ...
        if (
          ...
          message.origin === `${document.location.protocol}//${document.location.host}` &&
          !processingCode
        ) {
          processingCode = true
